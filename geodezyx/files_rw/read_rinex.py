...
def read_rinex2_obs(rnx_in,
    ...
    obs_all_list = [e for sublist in [(e,e+"_LLI",e+"_SSI") for e in obs_all_list] for e in sublist]
    ...
    df_rnx_obs = df_rnx_obs.reindex(["epoch","sys","prn"] + list(sorted(obs_all_list)),axis=1)
...
def read_rinex3_obs(rnx_in,
    ...
    for iepoc in tqdm(range(len(epochs)),desc="Reading " + filename):
        ...
        for sys in dict_sys_use.keys():
            #DFepoch_sys = df_epoch[df_epoch[0].str[0] == sys]
            #                   get the sats of the system sys  ||  get the meaningful columns
            df_epoch_sys_clean = df_epoch[df_epoch[0].str[0] == sys].iloc[:,:len(dict_sys_use[sys])]
            df_epoch_sys_clean.columns = dict_sys_use[sys]
        ...
        df_epoch_ok = pd.concat([col_epoch,col_sys,df_epoch_ok],axis=1)
                
        df_all_stk.append(df_epoch_ok)
    
    ## final concat and cosmetic (reorder columns, sort)
    df_rnxobs = pd.concat(df_all_stk)
    #Col_names = list(df_rnxobs.columns)
    #Col_names.remove("epoch")
    #Col_names.remove("prn")
    #Col_names = ["epoch","prn"] + sorted(Col_names)
    #df_rnxobs = df_rnxobs.reindex(Col_names,axis=1)
    
    df_rnxobs["prni"] = df_rnxobs["prn"].str[1:].astype(int)

    
    df_rnxobs.reset_index(drop=True,inplace=True)
    
    if set_index:
        df_rnxobs.set_index(set_index,inplace=True)
        df_rnxobs.sort_index(inplace=True)
...
def observables_dict_per_sys(df_rnx_in):
    ...
    for sys in df_rnx_in["sys"].unique():
        ...
        obs_sys = obs_sys0.index[obs_sys0][3:] ### strating from 3 to clean epoch sys prn
