...
def DFclk_filter(DFclk_in,
    ...
    name : iterable of str, optional
        List of wished satellites/stations. The default is None.
    ...
    if type(DFclk_in) is str:
        DFclk_wrk = utils.pickle_loader(DFclk_in)
    else:
        DFclk_wrk = DFclk_in
    
    BOOL = np.ones(len(DFclk_wrk)).astype(bool)

    
    if typ:
        BOOLtmp = DFclk_wrk.type.isin(typ)
        BOOL    = BOOL & np.array(BOOLtmp)

    if name:
        BOOLtmp = DFclk_wrk.name.isin(name)
        BOOL    = BOOL & np.array(BOOLtmp)        
        
    if ac:
        BOOLtmp = DFclk_wrk.ac.isin(ac)
        BOOL    = BOOL & np.array(BOOLtmp)    
        
        
    ##epoch
    BOOLtmp = (epoch_strt <= DFclk_wrk.epoch) & (DFclk_wrk.epoch <= epoch_end)
    BOOL    = BOOL & np.array(BOOLtmp)    
    
    
    return DFclk_wrk[BOOL]
