...
def read_clk(file_path_inp, names_4char=False):
    ...
    if (
        clk_df["bias"].dtype == "O"
        and clk_df["bias"].str.match(r".*D(\+|-)\d\d$").any()
    ):
    ...
    clk_df["epoch"] = pd.to_datetime(
        clk_df[["year", "month", "day", "hour", "minute", "second"]]
    )
...
def clk_diff(file_1, file_2):
    ...
    diff_df = pd.DataFrame(
        (list(zip(name_list_stk, type_list_stk, epoc_common_stk, d_clk_stk))),
...
def read_sp3(
    ...
    if not new_col_names:
        log.warning(
            "you use old column names (not conventional) set new_col_names as True and adapt your code"
        )
        ### DeprecationWarning
        col_names = ["epoch", "sat", "const", "sv", "type", "x", "y", "z", "clk", "AC"]
    else:
        col_names = ["epoch", "prn", "sys", "prni", "rec", "x", "y", "z", "clk", "ac"]
...
def read_sp3_header(sp3_inp, ac_name_inp=None):
    ...
    header_df = pd.DataFrame(
        list(zip(ac_name_list, sat_prn_list_final, sat_sig_list_final, date_list)),
