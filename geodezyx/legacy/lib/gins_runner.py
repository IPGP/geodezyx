...
def write_oceanload_file(station_file,oceanload_out_file,fes_yyyy=2004):
    ...
    p = subprocess.Popen(exe_loadoce_cmd + ' < ' + temp_cmd_fil, shell=True ,
                         executable='/bin/bash',
...
def gen_dirs_from_rnxs(rinex_paths_in,director_generik_path,
    ...
    for i,rinex_path in enumerate(rinex_path_lis):
        ...
        if auto_staocl:
            ...
            while not (bool_statfil and bool_oclofil) or iwhile < 6:
                if iwhile > 6:
                    print("WARN : something is going wrong in the loop waiting for the ocean loading file ...",iwhile)
                iwhile = iwhile + 1
                time.sleep(.5)
                bool_statfil = os.path.isfile(stations_file)
                bool_oclofil = os.path.isfile(oceanload_file)
...
def dirs_copy_generik_2_working(director_name_prefix,
    ...
    director_generik_path_tmp = os.path.join(temp_data_folder,
                                             os.path.basename(director_generik_path))
    director_generik_path_out = os.path.join(temp_data_folder,
                                             director_name_prefix + ".yml")   
    os.rename(director_generik_path_tmp,
              director_generik_path_out)
