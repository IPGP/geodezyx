...
def get_accur_coeff(i):
    ...
    ----------
    i : int
        Accuracy order index. Values > 3 return the highest accuracy coefficients.
    ...
    """
    accur_coeff_mat = np.array(
        [
            [0, 0, 0, -1 / 2.0, 0, 1 / 2.0, 0, 0, 0],
            [0, 0, 1 / 12.0, -2 / 3.0, 0, 2 / 3.0, -1 / 12.0, 0, 0],
            [0, -1 / 60.0, 3 / 20.0, -3 / 4.0, 0, 3 / 4.0, -3 / 20.0, 1 / 60.0, 0],
            [
                1 / 280.0,
                -4 / 105.0,
                1 / 5.0,
...
def partial_derive(f, var_in, var_out=0, kwargs_f={}, args_f=[], h=0, accur=-1):
    ...
    ----------
    f : callable
        The Python function to differentiate. Must return a scalar or iterable.
        Parameters susceptible to be differentiated must be scalars.
        For instance, to differentiate a position vector X = [x,y,z],
        f must take arguments f(x,y,z) not f(X).
    var_in : int or str
        The variable with respect to which the derivation is performed.
        Can be an int (starting with 0) or a string describing the name
        of the variable in f's arguments.
    var_out : int, optional
        The output index of f to consider. Default is 0.
    kwargs_f : dict, optional
        Dictionary describing keyword arguments of f. Default is {}.
    args_f : iterable, optional
        Tuple/list describing positional arguments of f. Default is [].
    h : float, optional
        Derivation step. If h == 0, uses x * sqrt(epsilon).
        Default is 0. See http://en.wikipedia.org/wiki/Numerical_differentiation
    accur : int, optional
        Accuracy coefficient index. -1 provides best accuracy (slowest).
        Default is -1. See https://en.wikipedia.org/wiki/Finite_difference_coefficient
    ...
    Notes
    -----
    The function automatically adjusts h if it equals 0 using:
    h = x * sqrt(machine_epsilon)
...
def jacobian_line(f, var_in_list, var_out=0, kwargs_f={}, args_f=[], h=0, aray=True):
    ...
    ----------
    f : callable
        The Python function to differentiate.
    var_in_list : iterable
        List/tuple of variables with respect to which derivation is performed.
    var_out : int, optional
        The output index of f to consider. Default is 0.
    kwargs_f : dict, optional
        Dictionary of keyword arguments for f. Default is {}.
    args_f : iterable, optional
        List/tuple of positional arguments for f. Default is [].
    h : float, optional
        Derivation step. Default is 0 (auto-calculated).
    aray : bool, optional
        If True, return result as numpy array. If False, return as list. Default is True.
    ...
    See Also
    --------
    partial_derive : Compute single partial derivative
    """
    line_out = []
...
def kwargs_for_jacobian(kwdic_generik, kwdic_variables):
    ...
    ----------
    kwdic_generik : dict
        Dictionary of parameters that will not change across computations.
    kwdic_variables : dict
        Dictionary of parameters that will change, with iterables as values.
        Each key should map to an iterable of values to be tested.

    Returns
    -------
    list of dict
        List of keyword argument dictionaries with all combinations of
        variables from kwdic_variables merged with kwdic_generik.
...
def jacobian(f, var_in_list, var_out, kwargs_f_list=[], h=10**-6, nproc=4):
    ...
    ----------
    f : callable
        The function for which to compute the Jacobian.
    var_in_list : iterable
        List of variables with respect to which derivation is performed.
    var_out : int
        Output index to consider.
    kwargs_f_list : list of dict, optional
        List of keyword argument dictionaries. Default is [].
    h : float, optional
        Derivation step. Default is 10**-6.
    nproc : int, optional
        Number of processes for parallel computation. Default is 4.
    ...
    Notes
    -----
    Only keyword arguments (kwargs) are managed. Positional arguments are
...
def nan_cleaner(Ain, Bin):
    ...
    -------
    Aout : ndarray
        First array with NaN values removed.
    Bout : ndarray
        Second array with NaN values removed at the same indices as Ain.
...
def weight_mat_simple(Pinp, Ninp=[], sparsediag=False, return_digaonal_only=False):
    """
...
def fuv_calc(V, A, P=1, normafuv=1):
    ...
    A : ndarray
        Jacobian matrix.
    P : int, float, or ndarray, optional
        Weight matrix or weight value. Can be a scalar, array, or sparse matrix.
        Default is 1.
    normafuv : int or float, optional
        Normalization factor for the FUV. Default is 1.
...
def sigmas_formal_calc(N, V, A, fuv=None, P=None):
    ...
    ----------
    N : ndarray
        Normal equation matrix (A^T * P * A).
    V : array-like
        Residuals vector.
    A : ndarray
        Jacobian matrix.
    fuv : float, optional
        Unitary variance factor. If None, it will be calculated. Default is None.
    P : ndarray, optional
        Weight matrix or array. Default is None.
    ...
    See Also
    --------
    fuv_calc : Calculate the unitary variance factor
    """
    if not fuv:
        fuv = fuv_calc(V, A, P)
...
def smart_i_giver(
    subgrp_len_list, i_in_sublis, sublis_id, advanced=False, sublis_id_list=[]
):
    """
    ...
    Parameters
    ----------
    subgrp_len_list : list
        List of lengths for each subgroup.
    i_in_sublis : int
        Local index within the subgroup.
    sublis_id : int or hashable
        The identifier of the subgroup to access.
    advanced : bool, optional
        If True, sublis_id is a generic identifier (str, int, set, etc.)
        present in sublis_id_list. If False, sublis_id is an integer index.
        Default is False.
    sublis_id_list : list, optional
        List of subgroup identifiers (used when advanced=True). Default is [].
...
def triangle_arr2vect(triarrin, k=1):
    ...
    ----------
    triarrin : ndarray
        Input 2D triangular array.
    k : int, optional
        Diagonal offset. k=0 includes the main diagonal, k=1 excludes it.
        Default is 1.
    ...
    See Also
    --------
    numpy.triu_indices_from : Get indices of upper triangle
    """
...
def bins_middle(bin_edges):
    ...
    bin_edges : array-like
        Bin edge values from numpy.histogram.
    ...
    for i in range(len(bin_edges) - 1):
        bin_edges2.append((bin_edges[i] + bin_edges[i + 1]) / 2.0)
    return bin_edges2
...
def chi2_test_frontend(dist_inp, nbins=10, ddof=2, debug=0, mode2=False, aaa=1):
    ...
    nbins : int, optional
        Number of bins for histogram. Default is 10.
    ddof : int, optional
        Degrees of freedom correction. Default is 2.
    debug : int, optional
        If non-zero, plot debug visualization. Default is 0.
    mode2 : bool, optional
        Normalization mode. If False (default), normalize theoretical distribution.
        If True, normalize observed distribution (less common, may be incoherent).
        Default is False.
    aaa : float, optional
        Scaling factor for standard deviation. Default is 1.
    ...
    Warnings
    --------
    The method for generating theoretical values is somewhat heuristic.
    See MATLAB's chi2gof.m line 185 for reference.

    Notes
    -----
    In debug mode, returns: bin_edges, bin_edges2, hist, gauss, chi2
    Otherwise returns chi2 statistic and p-value only.
    """
    ...
    cdf = scipy.stats.norm.cdf(
        bin_edges[1:-1], np.mean(dist_inp), np.std(dist_inp) * aaa
    )
...
def chi2_test_lsq(
    ...
    P : array-like, optional
        Weight matrix (diagonal only). Default is None.
    fuvin : float, optional
        Unitary variance factor. If None, will be calculated. Default is None.
    risk : float, optional
        Significance level for chi-square test. Default is 0.05.
    cleaning_std : bool, optional
        If True, clean data by removing standard deviation outliers. Default is False.
    cleaning_normalized : bool, optional
        If True, clean data using normalized residuals (preferred over cleaning_std).
        Default is False.
    koefP : float, optional
        Coefficient applied to P for finding viable solution. Default is 1.
    ...
    Cleaning options are tricks to approach FUV=1 by removing worst values.
    cleaning_normalized is preferred and overrides cleaning_std.
...
def error_ellipse(xm, ym, sigx, sigy, sigxy, nsig=1, ne=100, scale=1):
    ...
    if std1 < std2:
        ...
        dxy = np.dot(
            D, np.vstack((np.hstack((z1, np.flipud(z1))), np.hstack((z2, -z2))))
        )
    else:
        z2 = np.linspace(-std2, std2, ne)
        z1 = np.sqrt(V[0] * (np.ones(ne) - (z2 / std2) ** 2))
        dxy = np.dot(
            D, np.vstack((np.hstack((z1, -z1)), np.hstack((z2, np.flipud(z2)))))
        )
...
def error_ellipse_parameters(qxx, qyy, qxy, fuv, out_t=False):
    ...
    ----------
    qxx : float
        Variance in x direction (from variance-covariance matrix, unnormalized).
    qyy : float
        Variance in y direction (from variance-covariance matrix, unnormalized).
    qxy : float
        Covariance between x and y (from variance-covariance matrix, unnormalized).
    fuv : float
        Unitary variance factor for normalization.
    out_t : bool, optional
        If False (default), return angle in trigonometric convention (counterclockwise from x-axis).
        If True, return angle in clockwise direction from y-axis.
        Default is False.

    Returns
    -------
    Su : float
        Semi-major axis (scaled by FUV).
    Sb : float
        Semi-minor axis (scaled by FUV).
    t or phi : float
    ...
    This function calculates the principal axes and orientation of the
    2D confidence ellipse from the variance-covariance matrix.
...
def error_ellipse_parameters_2(sigx, sigy, sigxy, out_deg=True):
    ...
    ----------
    sigx : float
        Standard deviation in x direction (must be normalized with FUV).
    sigy : float
        Standard deviation in y direction (must be normalized with FUV).
    sigxy : float
        Covariance between x and y (must be normalized with FUV).
    ...
    Warnings
    --------
    The (X,Y) orientation convention is inverted from standard use, so in practice
    you must invert X and Y inputs. This is not important for the axes magnitudes
    but IS important for the orientation angle.
    ...
    l1 = 0.5 * (
        sigx2 + sigy2 + np.sqrt((sigx2 + sigy2) ** 2 - 4 * (sigx2 * sigy2 - sigxy2))
    )
    l2 = 0.5 * (
        sigx2 + sigy2 - np.sqrt((sigx2 + sigy2) ** 2 - 4 * (sigx2 * sigy2 - sigxy2))
    )
...
def ellipse_get_coords(
    a=0.0, b=0.0, x=0.0, y=0.0, angle=0.0, k=2, out_separate_X_Y=True, trigo=True
):
    """
    ...
    alpha = np.radians(np.r_[0.0 : 360.0 : 1j * (360 * k + 1)])
...
def ellipse_center(a):
    ...
    ----------
    a : array-like
        Ellipse equation coefficients [a, b, c, d, f, g] from the general
        ellipse equation: a*x^2 + 2*b*xy + c*y^2 + 2*d*x + 2*f*y + g = 0
...
def clean_nan(A, L):
    """
    .. deprecated::
        This function is discontinued. Use :func:`nan_cleaner` instead.
    ...
    -------
    A_clean : ndarray
        2D array with NaN values removed.
    L_clean : ndarray
        1D array with corresponding NaN values removed.
...
def fuv_calc_OLD2(V, A, P=None):
    ...
    A : ndarray
        Jacobian matrix.
    P : ndarray, optional
        Weight matrix. Default is None (identity matrix).
...
def partial_derive_old(f, var_in, var_out=0, kwargs_f={}, args_f=[], h=0):
    """
    .. deprecated::
        Use :func:`partial_derive` instead.
    ...
    f : callable
        The Python function to differentiate.
    var_in : int or str
        Variable with respect to which derivation is performed.
        Can be an int (starting with 0) or a string name.
    var_out : int, optional
        The output index of f to consider. Default is 0.
    kwargs_f : dict, optional
        Dictionary of keyword arguments for f. Default is {}.
    args_f : iterable, optional
        Tuple/list of positional arguments for f. Default is [].
    h : float, optional
        Derivation step. If h == 0, uses x * sqrt(epsilon). Default is 0.
