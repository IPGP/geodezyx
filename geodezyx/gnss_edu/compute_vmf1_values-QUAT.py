...
# -*- coding: utf-8 -*-
#
# Author: Samuel Nahmani (extended)
# Description: Compute VMF1 mapping values (hydro & wet) for a station/epoch/elevations
#              AND also return zhd/zwd from the VMF1 grids (if present).
#
# Modes:
#  - Simple (default): --epoch + --elev ... → prints table to stdout
#  - Batch: --pairs <2-col file epoch_iso,elev_deg> → vectorized; use --out to save CSV
#
# Notes:
#  - VMF1 grids dir default: Mapping_Fcn/vmf1 ; missing grids are auto-downloaded.
...
# Make sure path handling is robust on any OS
import builtins
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--out", type=Path,
        help=("Optional CSV output file (batch mode only).")
    )
    # VMF grids directory
    p.add_argument(
...
def main():
    ...
    if args.pairs:
        ...
        # Read all (ah,aw,zhd,zwd) in one shot
        ah_vec, aw_vec, zhd_vec, zwd_vec = rvg.read_vmf1_grid_ex(
            mjd=mjd_list,
            ell=[station["lat_deg"], station["lon_deg"], station["h_m"]]
        )
        ...
        for idx, dt in enumerate(unique_epochs_sorted):
            ...
            zhd = float(np.atleast_1d(zhd_vec)[idx]) if np.atleast_1d(zhd_vec).size else float("nan")
